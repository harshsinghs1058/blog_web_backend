    ...
    try {
      ...
      if (result) {
        if (result.password === password)
          res
            .status(202)
            .json({ message: "User Successfully Signed In", user: result });
        else {
      ...
      res
        .status(500)
        .json({ message: "An internal error has occurred", error: err });
    }
  } else {
...
signUp = async (req, res) => {
  ...
  if (result) {
    return res
      .status(409)
      .json({ message: `an account linked with "${email}" already exists` });
  }
